...
namespace DSSAlternative.AHP.Ratings
{
    public interface IRatingSystem
    {
        int Amount { get; set; }

        IEnumerable<(IRating from, IRating to)> RatingsFor(IRelationNode relation);
    }
    ...
    public class RatingSystem : IRatingSystem
    {
        ...
        public IEnumerable<(IRating from, IRating to)> RatingsFor(IRelationNode relation)
        {
