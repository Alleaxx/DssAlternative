...
namespace DSSAlternative.Web.AppComponents
{
    ...
    public interface IAccount
    {
        ...
        bool CheckEqualUserTemplate(IProject project);
    ...
    public class Account : IAccount
    {
        ...
        public Account(IProjectsCollection app, ILocalStorageService storage, IJsonService json, HttpClient httpClient, ILogger logger)
        {
            ...
            ProjectsCollection.OnSelectedProjectChanged += ProjectsCollection_OnSelectedProjectChanged;
        }
        private void ProjectsCollection_OnSelectedProjectChanged(IProject obj)
        {
            Logger.AddInfo(this, "Состояние сохранено", "Изменения в текущем проекте, сохраняемся", LogCategory.UI);
            SaveStateWithActiveProjects();
        }
        ...
        public bool CheckEqualUserTemplate(IProject project)
        {
            return TemplatesUserSaved.Any(x => x.Name.Equals(project.Name, StringComparison.CurrentCultureIgnoreCase));
        }
