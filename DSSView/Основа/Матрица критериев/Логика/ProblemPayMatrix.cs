...
namespace DSSView
{
    ...
    class ProblemPayMatrix
    {
        ...
        {
            Matrix = new MatrixObject(matrix);
    ...
    class ProblemSafeMatrix : ProblemPayMatrix
    {
        ...
        {
            MainMatrix = data;
            Matrix = new MatrixObject(new Matrix(GetSafeMatrix(data.Matrix.SourceMatrix.Arr)));
            InfoCriterias = new CriteriasReport(Matrix);
        }
    ...
    class MatrixAdvance<R, C, V>
    {
        ...
        //В виде списка ячеек
        public List<CellAdvance<V>> Cells { get; set; }
        private void UpdateCells()
        {
            Cells = new List<CellAdvance<V>>(ColsLen * RowsLen);
            for (int r = 0; r < RowsLen; r++)
            {
                for (int c = 0; c < ColsLen; c++)
                {
                    Cells.Add(GetCell(new Coords(r,c)));
                }
            }
        }
        ...
        public virtual CellAdvance<V> GetCell(Coords coords) => default;
    ...
    class Matrix : MatrixAdvance<Alternative, Case, double>
    {
        ...
        public override CellAdvance<double> GetCell(Coords coords) => new CellAdvanced(this, coords);
    ...
    class MatrixInformation
    {
        ...
        public MatrixInformation(Matrix data)
        {
            Matrix = data;
    ...
    class MatrixObject
    {
        ...
        {
            SourceMatrix = matrix;
            Info = new MatrixInformation(matrix);
            CreateRowsColumns();
            Name = $"{SourceMatrix.Rows}x{SourceMatrix.Cols}";
        ...
        public MatrixCol[] Colums { get; set; }
        private void CreateRowsColumns()
        {
            Rows = new MatrixRow[SourceMatrix.RowsLen + 1];
            for (int i = 0; i <= SourceMatrix.RowsLen; i++)
            {
                Rows[i] = new MatrixRow(this, i);
            }
            Colums = new MatrixCol[SourceMatrix.ColsLen + 1];
            for (int i = 0; i <= SourceMatrix.ColsLen; i++)
            {
                Colums[i] = new MatrixCol(this, i);
            }
        }

        private void UpdateRowsColumns()
        {
            CreateRowsColumns();
            StructureChanged?.Invoke();
        }
        public void SetValue(Coords coords, double val)
        {
            SourceMatrix.Arr[coords.X, coords.Y] = val;            
            ValuesChanged?.Invoke();
        }
    ...
    { 
        ...
        public MatrixContent(MatrixObject data,int pos)
        {
            Position = pos;
            Matrix = data;
            Cells = new List<Cell>();
    ...
    {
        public MatrixRow(MatrixObject data,int pos) : base(data, pos) { }
   ...
   /// </summary>
    ...
    {
        public MatrixCol(MatrixObject data,int pos) : base(data, pos) { }
        protected override void Fill()
        {
            if(Position == 0)
            {
    ...
    class CellAdvanced : CellAdvance<double>
    {
    ...
    abstract class Cell
    {
        ...
        public Cell(MatrixObject matrix, Coords coords)
        {
            Matrix = matrix;
            Coords = coords;
        }
    ...
    class CellValue : Cell
    {
        ...
        public CellValue(MatrixObject matrix, Coords coords) : base(matrix, coords)
        {
    ...
    class CellHeader : Cell
    {
        ...
        public CellHeader(MatrixObject matrix, Coords coords, CellHeaderInfo info) : base(matrix, coords)
        {
            Info = info;
        }
    }
    ...
    class CellName : Cell
    {
        ...
        public CellName(MatrixObject matrix) : base(matrix, new Coords(0, 0)) { }
    }
